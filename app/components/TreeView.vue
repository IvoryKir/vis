...
function onRowClick(row: { node: TreeNode }, event: MouseEvent) {
  ...
  // In Changes/Staged mode, clicking a modified file opens its git diff
  // instead of just selecting it. Untracked (?) and deleted (D) files fall
  // through to the regular select behavior.
  if (viewMode.value === 'changes' || viewMode.value === 'staged') {
    const status = displayStatus(row.node.path);
    if (status && status.code !== '?' && status.code !== 'D') {
      emit('select-file', row.node.path);
      emit('open-diff', { path: row.node.path, staged: status.staged });
      return;
    }
  }
...
.tree-tab + .tree-tab {
  border-left: 1px solid color-mix(in srgb, var(--text-faint) 35%, transparent);
}
...
.tree-statusbar {
  ...
  padding: 4px 8px;
  border-top: 1px solid color-mix(in srgb, var(--text-faint) 28%, transparent);
  flex-shrink: 0;
}
